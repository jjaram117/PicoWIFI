...
outer_led = DigitalInOut(board.GP28)
outer_led.switch_to_output()
...
def outer_led_msg(client, topic, message):
    # Method called whenever user/feeds/led has a new value
    print("New message on topic {0}: {1} ".format(topic, message))
    if message == "ON":
        outer_led.value = True
    elif message == "OFF":
        outer_led.value = False
    else:
        print("Unexpected message on LED feed.")
...
io.add_feed_callback("outerled", outer_led_msg)
